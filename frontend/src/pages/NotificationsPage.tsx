      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[var(--border)] pb-4">
        ...
        {notifications.filter(n => !n.isRead).length > 0 && (
          <button 
            ...
            className="px-4 py-2 border border-[var(--teal-border)] bg-[var(--teal-bg)] text-[var(--teal-text)] hover:bg-[var(--surface-hover)] rounded-lg text-xs font-semibold transition-colors"
          >
            ✔️ Mark All Read
          </button>
        )}
            ...
            >
              ...
              <div className="flex-1 space-y-1 text-xs">
                <div className="flex justify-between items-start gap-4">
                  <h4 className={`text-sm font-semibold text-[var(--text-primary)] ${!n.isRead ? 'font-bold' : ''}`}>
                    {n.title}
                  </h4>
                  <div className="flex gap-2">
                    {!n.isRead && (
                      <button 
                        ...
                        className="text-[10px] text-[var(--teal-text)] hover:underline"
                      >
                        Mark as read
                      </button>
                    )}
                    <button 
                      ...
                      className="text-[10px] text-[var(--red-icon)] hover:underline"
                    >
                      Delete
                    </button>
                  </div>
