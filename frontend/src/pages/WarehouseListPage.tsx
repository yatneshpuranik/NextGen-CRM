      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-4 py-2.5 bg-[var(--purple-bg)] border border-[var(--purple-icon)] text-[var(--purple-text-strong)] font-semibold text-xs rounded-lg hover:bg-[var(--surface-hover)] transition-colors"
          >
            🔄 Transfer Stock
          </button>
          <button
            onClick={() => navigate('/dashboard/warehouses/new')}
            className="px-4 py-2.5 bg-[var(--teal-bg)] border border-[var(--teal-border)] text-[var(--teal-text-strong)] font-semibold text-xs rounded-lg hover:bg-[var(--surface-hover)] transition-colors"
          >
            ➕ Add Warehouse
          </button>
        </div>
              ...
              >
                <div className="space-y-2">
                  ...
                  <div className="text-xs text-[var(--text-secondary)] space-y-1">
                    <p>📍 {wh.address}</p>
                    <p>👤 Contact: <span className="font-semibold text-[var(--text-primary)]">{wh.contactPerson}</span> ({wh.contactNumber})</p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-[var(--border)] flex gap-2 justify-end">
                  <button
                    ...
                    className="px-3 py-1.5 border rounded-lg text-xs font-medium hover:bg-[var(--surface-hover)]"
                  >
                    ✏️ Edit
                  </button>
                  <button
                    onClick={() => navigate(`/dashboard/warehouses/${wh.id}`)}
                    className="px-3 py-1.5 bg-[var(--teal-bg)] border border-[var(--teal-border)] text-[var(--teal-text)] rounded-lg text-xs font-semibold hover:bg-[var(--surface-hover)]"
                  >
                    🔍 View Details
                  </button>
                </div>
